describe('Xóa tất cả mã giảm giá', () => {
	function deleteFirstItem() {
		...
		cy.get('.k-master-row.ng-scope')
			.first()
			.then($item => {
				if (Cypress._.isEmpty($item)) {
					cy.log('Button không tồn tại, thoát bài test.');
					return; // Dừng bài kiểm tra sớm
				}
	...
	it('Xóa tất cả nhân viên trừ mã giảm giá cuối cùng', () => {
		// mở web
		...
		cy.intercept('GET', '/api/users?format=json&%24inlinecount=allpages&%24top=20').as(
			'getData'
		);
